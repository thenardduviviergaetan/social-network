...
func GetUserByUUID(db *sql.DB, uuid string) (*models.User, error) {
	user := &models.User{}
	err := db.QueryRow("SELECT uuid, email, password, first_name, last_name, date_of_birth, status, nickname, about FROM users WHERE uuid=?", uuid).Scan(
		&user.UUID,
		&user.Email,
		&user.Password,
		&user.FirstName,
